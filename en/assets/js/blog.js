document.addEventListener('DOMContentLoaded', function () {
  ...
  function loadCategories(activeCategory = null) {
    fetch(`${API_BASE}/categories/en`)
      ...
      .catch(err => {
        console.error('Erreur Loading catégories:', err);
      });
  }
  ...
  function loadBlogPosts(categorySlug = null, tagSlug = null) {
    ...
    fetch(url)
      ...
      .catch(err => {
        ...
        if (blogGrid) {
          blogGrid.innerHTML = '<p class="text-center">No articles found.</p>';
        }
      });
  }
...
function loadFeaturedBlogs() {
  ...
  const url = `${API_BASE}/blogs/en?featured=true`;
  ...
  fetch(url)
    ...
    .catch(err => {
      console.error('Erreur Loading featured blogs:', err);
      ...
      <article class="blog-card">
          ...
          <div class="blog-card__body">
            <div class="blog-card__date">
              ${new Date(post.publish_date).toLocaleDateString('en-EN')}
            </div>
