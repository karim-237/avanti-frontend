...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // =======================================================
  // 🔹 1) Loading DU BLOG PRINCIPAL
  // =======================================================
  if (!slug) {
    console.error("❌ Slug manquant");
    const contentEl = document.getElementById("blog-content");
    if (contentEl) contentEl.innerHTML = "<p>Blog not found (missing slug).</p>";
    return;
  }

  fetch(`${API_BASE}/blogs/en/${slug}`)
    .then(res => {
    ...
    .then(data => {
      ...
      if (!data.success) {
        ...
        const contentEl = document.getElementById("blog-content");
        if (contentEl) contentEl.innerHTML = `<p>Blog not found : ${data.message}</p>`;
        return;
      }
      ...
      if (!data.data || !data.data.blog) {
        ...
        const contentEl = document.getElementById("blog-content");
        if (contentEl) contentEl.innerHTML = "<p>Blog not found.</p>";
        return;
      }
      ...
      if (titleEl) titleEl.textContent = blog.title || "Title unavailable";
      ...
      if (dateEl && blog.publish_date) {
        dateEl.textContent = new Date(blog.publish_date).toLocaleDateString("en-EN", {
          day: "2-digit",
          month: "long",
          year: "numeric",
        });
      ...
      if (featuredEl) {
        featuredEl.innerHTML = featured.length
          ? `<h4>Featured</h4>${featured.map(
            f => `
              <div class="feed">
                <img src="${f.image_url}" class="img-fluid" alt="${f.title}">
                <a href="/en/single-blog.html?slug=${f.slug}">${f.title}</a>
              </div>
            `
          ).join("")}`
          : "<h4>Featured</h4><p>No blogs</p>";
      }
    ...
    .catch(err => {
      ...
      const contentEl = document.getElementById("blog-content");
      if (contentEl) contentEl.innerHTML = "<p>Error loading blog.</p>";
    });

  // =======================================================
  // 🔹 2) Loading DES 5 DERNIERS BLOGS
  // =======================================================
  const latestBlogsEl = document.getElementById("latest-blogs"); 

  if (latestBlogsEl) {
    fetch(`${API_BASE}/blogs/en/latest`)
      .then(res => {
      ...
      .then(data => {
        if (!data.success || !Array.isArray(data.data)) {
          ...
          latestBlogsEl.innerHTML = "<li>No items available</li>";
          return;
        }
            ...
            b => `
              <li class="d-flex mb-3">
                ...
                <div style="text-align: left; flex: 1;">
                  <a href="/en/single-blog.html?slug=${b.slug}" class="fw-bold d-block">
                    ${b.title}
                  </a>
                  <small class="text-muted">
                    ${new Date(b.publish_date).toLocaleDateString("en-EN", {
              day: "2-digit",
              month: "short",
              year: "numeric",
            })}
      ...
      .catch(err => {
        ...
        latestBlogsEl.innerHTML = "<li>Error Loading</li>";
      });
  } else {
