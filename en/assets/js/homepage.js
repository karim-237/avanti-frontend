...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function initHeroCarousel() {
    fetch(`${API_BASE}/home-banners/en`)
      .then(res => {
  ...
  function initTildaProducts() {
    Promise.all([
      fetch(`${API_BASE}/product-categories/en`).then(r => {
        if (!r.ok || r.headers.get('content-type')?.includes('text/html')) return { data: [] };
        return r.json().catch(() => ({ data: [] }));
      }),
      fetch(`${API_BASE}/products/en?featured=true&limit=8`).then(r => {
        if (!r.ok || r.headers.get('content-type')?.includes('text/html')) return { data: [] };
        return r.json().catch(() => ({ data: [] }));
      })
      ...
      if (categoryGrid) {
        ...
        if (mainCategories.length === 0) {
          ...
          categoryGrid.innerHTML = mainCategories.map(cat => {
            ...
            return `
            <a href="/en/products.html?category=${cat.slug || ''}" class="category-card">
      ...
      if (productsList) {
        if (!Array.isArray(products) || products.length === 0) {
          productsList.innerHTML = '<li class="splide__slide"><p class="text-center">No products available at the moment.</p></li>';
          return;
        }
          ...
          <li class="splide__slide">
            <div class="card">
              ...
              <div class="card__body">
                ...
                <a href="/en/product-detail.html?slug=${product.slug || ''}" class="btn btn--primary btn--sm" style="margin-top: auto;">View product</a> 
              </div>
            </div>
          </li>
        `;
  ...
  function initRecipes() {
    fetch(`${API_BASE}/recipes/en?featured=true&limit=3`)
      .then(res => {
      ...
      .then(recipesRes => {
        ...
        if (!Array.isArray(recipes) || recipes.length === 0) {
          grid.innerHTML = '<p>No recipes available at the moment.</p>';
          return;
        }
        ...
        grid.innerHTML = recipes.map(recipe => {
          ...
          <div class="recipe-card">
            ...
            <div class="recipe-card__body">
              <span class="recipe-card__badge">${recipe.category || 'Recipe'}</span>
              <h3 class="recipe-card__title">${recipe.title || 'Recipe'}</h3>
              <p class="recipe-card__text">${recipe.short_description || ''}</p>
              <a href="/en/single-recipe.html?slug=${recipe.slug || ''}" class="btn btn--secondary btn--sm">See the recipe</a>
            </div>
          </div>
        `;
        }).join('');
  ...
  function initBlog() {
    fetch(`${API_BASE}/blogs/en?featured=true&limit=3`)
      .then(res => {
        if (!res.ok || res.headers.get('content-type')?.includes('text/html')) return { data: [] };
        return res.json().catch(() => ({ data: [] }));
      })
        ...
        grid.innerHTML = posts.map(post => {
          ...
          <div class="blog-card">
            ...
            <div class="blog-card__body">
              <div class="blog-card__date">${post.published_at || post.publish_date ? new Date(post.published_at || post.publish_date).toLocaleDateString('en-EN') : ''}</div>
              ...
              <a href="/en/single-blog.html?slug=${post.slug || ''}" class="btn btn--primary btn--sm">Read the article</a>
            </div>
          </div>
        `;
        }).join('');
  ...
  function initInstagram() {
    fetch(`${API_BASE}/instagram-feed`)
      ...
      .catch(err => {
        ...
        const grid = document.getElementById('instagramGrid');
        if (grid) {
          grid.innerHTML = '<p>Loading des images Instagram...</p>';
        }
      });
  }
