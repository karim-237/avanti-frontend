document.addEventListener('DOMContentLoaded', function() {
  ...
  function initCategories() {
    ...
    fetch(`${API_BASE}/recipes/en/recipe-categories`)
      .then(res => res.json())
      .then(result => {
        if (!result.success || !Array.isArray(result.data)) return;
        renderCategories(result.data);
      })
      .catch(err => console.error('Erreur Loading catégories:', err));
  }
  ...
  function loadRecipes(category = null, tag = null) {
    ...
    fetch(url)
      ...
      .catch(err => {
        ...
        if (recipesGrid) recipesGrid.innerHTML = '<p class="text-center">No recipes available at the moment.</p>';
      });
  }
  ...
  function displayRecipes(recipes) {
    ...
    if (!Array.isArray(recipes) || recipes.length === 0) {
      recipesGrid.innerHTML = '<p class="text-center">No recipe found.</p>';
      return;
    }
