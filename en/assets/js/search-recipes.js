...
$(document).ready(function () {
  ...
  function renderResults(results) {
    ...
    if (!results || results.length === 0) {
      resultsDropdown
        .html('<p style="padding:8px;color:#666;">No results</p>')
        .show();
      positionDropdown();
      return;
    }
...
if (latestTitle.length) {
  latestTitle.text("Recent recipes"); // changement du titre
}
...
async function loadLatestRecipes(limit = 5) {
  ...
  try {
    const response = await fetch(`${API_BASE}/recipes/en/latest?limit=${limit}`);
    if (!response.ok) return;
    ...
    if (!recipes.length) {
      latestRecipesContainer.html('<li>No recipes</li>');
      return;
    }
    ...
    console.error("Erreur Loading dernières recettes :", err);
    latestRecipesContainer.html('<li>Unable to load recipes</li>');
  }
}
